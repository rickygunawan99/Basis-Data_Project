...
class TransactionsRepository
{
    ...
    public function detail(int $month, int $year): ?array
    {
        $statement = $this->connection->prepare("SELECT COUNT(*) 'terjual', produser.nama_produser 'produser',
                                    ...
                                    GROUP BY id_products, total_harga
    ...
    {
        try {
            $statement = $this->connection->prepare("SELECT produser.nama_voucher 'voucher',
                                transactions.id 'id',
                               produser.nama_produser 'produser',
                               products.description 'descr',
                               products.jml_item 'nominal',
                               transactions.total_harga 'harga',
                               transactions.kode_voucher 'kode',
                               transactions.tanggal_pembelian 'tanggal',
                               transactions.status_pemesanan 'status'
                        FROM transactions
                        JOIN users ON (users.email = transactions.email)
                        JOIN products ON (products.id = transactions.id_products)
                        JOIN produser ON (produser.id = products.id_produser)
    ...
    public function countDetailAdmin(int $month, int $year):array
    {
        try {
            if($month == 0 && $year == 0){
                $statement = $this->connection->prepare("SELECT users.email 'email', 
                    transactions.email_admin 'email_admin', 
                    COUNT(email_admin) 'total'
                    FROM transactions
                    RIGHT JOIN users ON (users.email = transactions.email_admin)
                    GROUP BY email
                    ORDER BY total DESC");
                $statement->execute();
            }else{
                $statement = $this->connection->prepare("SELECT users.email 'email', 
                    ...
                    FROM transactions
                    RIGHT JOIN users ON (users.email = transactions.email_admin)
                    WHERE MONTH(tanggal_pembelian) = ? AND 
                          YEAR(tanggal_pembelian) = ?
                    GROUP BY email
                    ORDER BY total DESC");
                $statement->execute([$month,$year]);
            }
